...
#define FONT_READOUT 40
#define FONT_LABEL 24
#define FONT_BODY 18
#define FONT_MICRO 16

static Font hud_font = {0};
static Font hud_title_font = {0};

/* SFX volume knobs (0.0..1.0); engine pitch scales 1x idle -> ~3.4x max speed. */
#define SFX_ENGINE_VOLUME 0.28f
#define SFX_DRIFT_VOLUME 0.35f
#define SFX_SCRAPE_VOLUME 0.55f
#define SFX_CRASH_VOLUME 1.00f
#define SFX_PICKUP_VOLUME 0.80f
#define SFX_REFUEL_VOLUME 0.30f
#define SFX_LOWFUEL_VOLUME 0.60f
#define SFX_OUTOFFUEL_VOLUME 0.70f
#define SFX_WRECK_VOLUME 1.00f
#define SFX_RESTART_VOLUME 0.70f

static Sound s_engine = {0};
static Sound s_drift = {0};
static Sound s_crash = {0};
static Sound s_scrape = {0};
static Sound s_pickup = {0};
static Sound s_refuel = {0};
static Sound s_lowfuel = {0};
static Sound s_outoffuel = {0};
static Sound s_wreck = {0};
static Sound s_restart = {0};

static void DrawHudText(const char *text, int x, int y, int size, Color color)
{
    DrawTextEx(hud_font, text, (Vector2){(float)x, (float)y}, (float)size, 0.0f, color);
}

static int HudTextWidth(const char *text, int size)
{
    return (int)MeasureTextEx(hud_font, text, (float)size, 0.0f).x;
}

static int HudTitleWidth(const char *text, int size)
{
    return (int)MeasureTextEx(hud_title_font, text, (float)size, 0.0f).x;
}

static void DrawHudTitle(const char *text, int x, int y, int size, Color color)
{
    DrawTextEx(hud_title_font, text, (Vector2){(float)x, (float)y}, (float)size, 0.0f, color);
}

static void DrawCenterPlate(const char *text, int size, int y, Color text_color)
{
    int width = HudTitleWidth(text, size);
    DrawRectangle(
        GetScreenWidth() / 2 - width / 2 - 20,
        y,
        width + 40,
        size + 24,
        (Color){0, 0, 0, 180});
    DrawHudTitle(text, GetScreenWidth() / 2 - width / 2, y + 12, size, text_color);
}
...
void DrawBarGauge(const char *title, const char *hint, float value, float maximum, int x, int y)
{
    const int width = 260;
    const int height = 25;

    float ratio = ClampFloat(value / maximum, 0.0f, 1.0f);
    ...
    DrawHudText(title, x, y - 27, FONT_LABEL, WHITE);

    DrawRectangle(x + 4, y + 5, width, height, (Color){0, 0, 0, 100});

    DrawRectangle(x, y, width, height, (Color){25, 28, 35, 230});

    DrawRectangle(x + 3, y + 3, (int)((width - 6) * ratio), height - 6, bar_color);

    DrawRectangleLines(x, y, width, height, WHITE);

    const char *text = TextFormat("%.0f%%", ratio * 100.0f);
    int text_width = HudTextWidth(text, FONT_BODY);

    DrawHudText(text, x + width / 2 - text_width / 2, y + 3, FONT_BODY, WHITE);

    DrawHudText(hint, x, y + 35, FONT_BODY, (Color){230, 230, 230, 255});
}
    ...
    int screen_height)
{
    ...
    for (int i = 0; i <= 10; i++)
    {
        ...
        int tick_width = HudTextWidth(tick_text, FONT_MICRO);
    ...
    int speed_width = HudTextWidth(speed_text, FONT_READOUT);
    ...
    int unit_width = HudTextWidth(unit_text, FONT_MICRO);
    ...
    int gear_width = HudTextWidth(gear_text, FONT_LABEL);
...
int main(void)
{
    ...
    hud_font = LoadFontEx("fonts/rajdhani-regular.ttf", 64, NULL, 0);
    hud_title_font = LoadFontEx("fonts/rajdhani-bold.ttf", 64, NULL, 0);

    if (hud_font.glyphCount == 0)
        hud_font = GetFontDefault();
    if (hud_title_font.glyphCount == 0)
        hud_title_font = hud_font;

    InitAudioDevice();

    if (IsAudioDeviceReady())
    {
        s_engine = LoadSound("audio/engine.wav");
        s_drift = LoadSound("audio/drift.wav");
        s_crash = LoadSound("audio/crash.wav");
        s_scrape = LoadSound("audio/scrape.wav");
        s_pickup = LoadSound("audio/pickup.wav");
        s_refuel = LoadSound("audio/refuel.wav");
        s_lowfuel = LoadSound("audio/lowfuel.wav");
        s_outoffuel = LoadSound("audio/outoffuel.wav");
        s_wreck = LoadSound("audio/wreck.wav");
        s_restart = LoadSound("audio/restart.wav");

        SetSoundVolume(s_engine, SFX_ENGINE_VOLUME);
        SetSoundVolume(s_drift, SFX_DRIFT_VOLUME);
        SetSoundVolume(s_crash, SFX_CRASH_VOLUME);
        SetSoundVolume(s_scrape, SFX_SCRAPE_VOLUME);
        SetSoundVolume(s_pickup, SFX_PICKUP_VOLUME);
        SetSoundVolume(s_refuel, SFX_REFUEL_VOLUME);
        SetSoundVolume(s_lowfuel, SFX_LOWFUEL_VOLUME);
        SetSoundVolume(s_outoffuel, SFX_OUTOFFUEL_VOLUME);
        SetSoundVolume(s_wreck, SFX_WRECK_VOLUME);
        SetSoundVolume(s_restart, SFX_RESTART_VOLUME);
    }

    SetTextureFilter(hud_font.texture, TEXTURE_FILTER_BILINEAR);
    SetTextureFilter(hud_title_font.texture, TEXTURE_FILTER_BILINEAR);

    Image soil_image = LoadImage("img/Soil_Tile.png");
    ...
    */
    ...
    bool was_out_of_fuel = false;
    bool lowfuel_beep_on = false;
    ...
    while (!WindowShouldClose())
    {
        ...
        camera.offset.y = (float)screen_height * 0.68f;
            ...
            IsKeyPressed(KEY_ENTER))
        {
            ...
            was_wrecked = false;
            was_out_of_fuel = false;
            lowfuel_beep_on = false;
        }
            ...
            IsKeyDown(KEY_R))
        {
            ...
            if (!IsSoundPlaying(s_refuel))
                PlaySound(s_refuel);
        }
        else if (IsSoundPlaying(s_refuel))
        {
            StopSound(s_refuel);
        }
        ...
        */
        ...
        if (!was_wrecked && health <= 0.0f)
            PlaySound(s_wreck);

        was_wrecked = (health <= 0.0f);
        ...
        bool engine_audible =
            engine_has_fuel &&
            fabsf(car_speed) > 0.1f;

        SetSoundPitch(s_engine, 1.0f + speed_ratio * 2.4f);

        if (engine_audible && !IsSoundPlaying(s_engine))
            PlaySound(s_engine);
        else if (!engine_audible && IsSoundPlaying(s_engine))
            StopSound(s_engine);
        ...
        */
        ...
        if (drifting)
        {
            ...
            if (!IsSoundPlaying(s_drift))
                PlaySound(s_drift);
        }
        else if (IsSoundPlaying(s_drift))
        {
            StopSound(s_drift);
        }
        ...
        */
        ...
        DrawBarGauge("FUEL", "Hold R to refuel", fuel, maximum_fuel, 30, 75);

        DrawBarGauge("CAR HEALTH", "Collect green health kits to repair", health, maximum_health, 30, 165);
            ...
            fuel <= maximum_fuel * 0.20f)
        {
            ...
            if (show_warning)
            {
                DrawCenterPlate("LOW FUEL", FONT_TITLE, 24, ORANGE);

                if (!lowfuel_beep_on)
                    PlaySound(s_lowfuel);
            }

            lowfuel_beep_on = show_warning;
        }
        ...
        {
            if (!was_out_of_fuel)
                PlaySound(s_outoffuel);

            const char *title = "OUT OF FUEL";
            const char *hint = "HOLD R TO REFUEL";
            int title_width = HudTitleWidth(title, FONT_TITLE);
            int hint_width = HudTextWidth(hint, FONT_LABEL);
            int box_width = (title_width > hint_width ? title_width : hint_width) + 40;
            int box_height = FONT_TITLE + FONT_LABEL + 28;
        ...
        was_out_of_fuel = (fuel <= 0.0f && health > 0.0f);
            ...
            health <= maximum_health * 0.25f)
        {
            ...
            {
                DrawCenterPlate("CRITICAL CAR DAMAGE", FONT_LABEL, 80, RED);
            }
        }
        ...
        if (health <= 0.0f)
        {
            ...
            int wrecked_width =
                HudTitleWidth(wrecked, FONT_TITLE);

            int restart_width =
                HudTextWidth(restart, FONT_LABEL);
    ...
    if (IsAudioDeviceReady())
    {
        UnloadSound(s_engine);
        UnloadSound(s_drift);
        UnloadSound(s_crash);
        UnloadSound(s_scrape);
        UnloadSound(s_pickup);
        UnloadSound(s_refuel);
        UnloadSound(s_lowfuel);
        UnloadSound(s_outoffuel);
        UnloadSound(s_wreck);
        UnloadSound(s_restart);
